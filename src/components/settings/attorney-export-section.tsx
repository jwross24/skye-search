...
export function AttorneyExportSection() {
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await fetch('/api/attorney-export')
      ...
      a.download = res.headers.get('Content-Disposition')
        ?.match(/filename="(.+)"/)?.[1]
        ?? `attorney-export-${new Date().toISOString().split('T')[0]}.json`
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="text-sm text-foreground/60 leading-relaxed">
        Formatted for your immigration attorney. Includes your unemployment log,
        employment periods, application history, and weekly job search activity
        for USCIS compliance. Every date is labeled with its source.
      </p>
